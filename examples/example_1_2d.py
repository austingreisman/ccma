...
def generate_shapes(n, noise_sigma):
    ...
    # Split points evenly among segments where necessary
    n_third = n // 3
    n_quarter = n // 4
    n_half = n // 2
...
right_angle_true = shapes["WavyPath"]["true"]
right_angle_noisy = shapes["WavyPath"]["noisy"]
...
ccma_points = ccma.filter(right_angle_noisy, mode="padding")
# ccma_points_wo_padding = ccma.filter(right_angle_noisy, mode="padding")
